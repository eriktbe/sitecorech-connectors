...
namespace DynamicsWebAPISample
{
    public class SampleHelpers
    {
        ...
        public static void DisplayException(Exception ex, ILogger log)
        {
            log.LogError("The application terminated with an error.");
            log.LogError(ex.Message);
            while (ex.InnerException != null)
            {
                log.LogError("\t* {0}", ex.InnerException.Message);
